export const translations = {
  en: {
    // Navbar
    home: "Home",
    about: "About",
    projects: "Projects",
    skills: "Skills",
    experience: "Experience",
    contact: "Contact",
    
    // Intro
    introTitle: "Kilian Balaguer",
    introSubtitle: "15-Year-Old Developer & Creator",
    introDescription: "I build apps, games, and websites. I'm passionate about coding, music, and creating things that matter.",
    introBadge: "DEVELOPER / CREATOR",
    getInTouch: "Get in touch",
    resume: "Resume",
    
    // About
    aboutTitle: "About Me",
    aboutP1: "Hey! I'm Kilian, a 15-year-old developer from Belgium. I started coding when I was 7, and I've been hooked ever since. I love building apps, games, and websites that solve real problems.",
    aboutP2: "When I'm not coding, you'll find me making music. I play piano, guitar, bass, drums, and I produce my own tracks. Music and code share something special - they're both about creating something from nothing.",
    aboutP3: "I also love cooking and experimenting in the kitchen. Whether it's code, music, or food, I'm always learning and creating something new.",
    aboutMusic: "MUSIC",
    aboutMusicText: "Piano • Guitar • Bass • Drums",
    aboutInterests: "INTERESTS",
    aboutInterestsText: "Cooking • Game Dev • Music Production",
    aboutAge: "AGE",
    aboutAgeText: "15 years old",
    
    // Projects
    projectsTitle: "My Projects",
    sourceCode: "Source Code",
    liveDemo: "Live Demo",
    comingSoon: "Coming Soon",
    code: "CODE",
    live: "LIVE",
    
    // Skills
    skillsTitle: "Skills & Technologies",
    
    // Experience
    experienceTitle: "My Journey",
    
    // Contact
    contactTitle: "Contact Me",
    contactDescription: "Have a project in mind? Let's work together to create something amazing.",
    contactEmail: "EMAIL",
    namePlaceholder: "YOUR NAME",
    emailPlaceholder: "YOUR EMAIL",
    messagePlaceholder: "YOUR MESSAGE",
    send: "Send",
    successMessage: "Thank you for contacting us! We will get back to you as soon as possible.",
    
    // Footer
    footerRights: "All rights reserved.",
    footerAbout: "About this site",
    footerTech: "Built with React, Next.js, TypeScript, Tailwind CSS, Framer Motion, and hosted on Vercel.",
  },
  fr: {
    // Navbar
    home: "Accueil",
    about: "À propos",
    projects: "Projets",
    skills: "Compétences",
    experience: "Expérience",
    contact: "Contact",
    
    // Intro
    introTitle: "Kilian Balaguer",
    introSubtitle: "Développeur & Créateur de 15 ans",
    introDescription: "Je crée des applications, des jeux et des sites web. Je suis passionné par le code, la musique et la création de choses qui comptent.",
    introBadge: "DÉVELOPPEUR / CRÉATEUR",
    getInTouch: "Me contacter",
    resume: "CV",
    
    // About
    aboutTitle: "À propos de moi",
    aboutP1: "Salut ! Je suis Kilian, un développeur de 15 ans de Belgique. J'ai commencé à coder à 7 ans et je suis accro depuis. J'adore créer des applications, des jeux et des sites qui résolvent de vrais problèmes.",
    aboutP2: "Quand je ne code pas, je fais de la musique. Je joue du piano, de la guitare, de la basse, de la batterie, et je produis mes propres morceaux. La musique et le code partagent quelque chose de spécial - ils consistent tous deux à créer quelque chose à partir de rien.",
    aboutP3: "J'aime aussi cuisiner et expérimenter en cuisine. Que ce soit le code, la musique ou la cuisine, j'apprends toujours et je crée quelque chose de nouveau.",
    aboutMusic: "MUSIQUE",
    aboutMusicText: "Piano • Guitare • Basse • Batterie",
    aboutInterests: "INTÉRÊTS",
    aboutInterestsText: "Cuisine • Dev de Jeux • Production Musicale",
    aboutAge: "ÂGE",
    aboutAgeText: "15 ans",
    
    // Projects
    projectsTitle: "Mes Projets",
    sourceCode: "Code Source",
    liveDemo: "Démo",
    comingSoon: "Bientôt Disponible",
    code: "CODE",
    live: "DÉMO",
    
    // Skills
    skillsTitle: "Compétences & Technologies",
    
    // Experience
    experienceTitle: "Mon Parcours",
    
    // Contact
    contactTitle: "Me Contacter",
    contactDescription: "Vous avez un projet en tête ? Travaillons ensemble pour créer quelque chose d'incroyable.",
    contactEmail: "EMAIL",
    namePlaceholder: "VOTRE NOM",
    emailPlaceholder: "VOTRE EMAIL",
    messagePlaceholder: "VOTRE MESSAGE",
    send: "Envoyer",
    successMessage: "Merci de nous avoir contactés ! Nous vous répondrons dès que possible.",
    
    // Footer
    footerRights: "Tous droits réservés.",
    footerAbout: "À propos de ce site",
    footerTech: "Construit avec React, Next.js, TypeScript, Tailwind CSS, Framer Motion, et hébergé sur Vercel.",
  },
  ar: {
    // Navbar
    home: "الرئيسية",
    about: "عني",
    projects: "المشاريع",
    skills: "المهارات",
    experience: "الخبرة",
    contact: "اتصل بي",
    
    // Intro
    introTitle: "كيليان بالاغير",
    introSubtitle: "مطور ومبدع في سن 15",
    introDescription: "أقوم ببناء التطبيقات والألعاب والمواقع. أنا شغوف بالبرمجة والموسيقى وإنشاء أشياء مهمة.",
    introBadge: "مطور / مبدع",
    getInTouch: "تواصل معي",
    resume: "السيرة الذاتية",
    
    // About
    aboutTitle: "نبذة عني",
    aboutP1: "مرحباً! أنا كيليان، مطور يبلغ من العمر 15 عامًا من بلجيكا. بدأت البرمجة عندما كان عمري 7 سنوات، وأنا مدمن منذ ذلك الحين. أحب بناء التطبيقات والألعاب والمواقع التي تحل مشاكل حقيقية.",
    aboutP2: "عندما لا أكون أبرمج، ستجدني أصنع الموسيقى. أعزف على البيانو والجيتار والباس والطبول، وأنتج مقطوعاتي الخاصة. تشترك الموسيقى والبرمجة في شيء خاص - كلاهما يتعلق بإنشاء شيء من لا شيء.",
    aboutP3: "أحب أيضًا الطبخ والتجريب في المطبخ. سواء كان ذلك برمجة أو موسيقى أو طعام، فأنا دائمًا أتعلم وأخلق شيئًا جديدًا.",
    aboutMusic: "الموسيقى",
    aboutMusicText: "بيانو • جيتار • باس • طبول",
    aboutInterests: "اهتمامات",
    aboutInterestsText: "الطبخ • تطوير الألعاب • إنتاج الموسيقى",
    aboutAge: "العمر",
    aboutAgeText: "15 سنة",
    
    // Projects
    projectsTitle: "مشاريعي",
    sourceCode: "الكود المصدري",
    liveDemo: "العرض المباشر",
    comingSoon: "قريباً",
    code: "كود",
    live: "مباشر",
    
    // Skills
    skillsTitle: "المهارات والتقنيات",
    
    // Experience
    experienceTitle: "رحلتي",
    
    // Contact
    contactTitle: "اتصل بي",
    contactDescription: "لديك مشروع في ذهنك؟ لنعمل معًا لإنشاء شيء مذهل.",
    contactEmail: "البريد الإلكتروني",
    namePlaceholder: "اسمك",
    emailPlaceholder: "بريدك الإلكتروني",
    messagePlaceholder: "رسالتك",
    send: "إرسال",
    successMessage: "شكرًا للتواصل معنا! سنرد عليك في أقرب وقت ممكن.",
    
    // Footer
    footerRights: "جميع الحقوق محفوظة.",
    footerAbout: "حول هذا الموقع",
    footerTech: "تم البناء باستخدام React و Next.js و TypeScript و Tailwind CSS و Framer Motion، واستضافته على Vercel.",
  },
  nl: {
    // Navbar
    home: "Home",
    about: "Over",
    projects: "Projecten",
    skills: "Vaardigheden",
    experience: "Ervaring",
    contact: "Contact",
    
    // Intro
    introTitle: "Kilian Balaguer",
    introSubtitle: "15-jarige Ontwikkelaar & Maker",
    introDescription: "Ik bouw apps, games en websites. Ik ben gepassioneerd over coderen, muziek en het creëren van dingen die ertoe doen.",
    introBadge: "ONTWIKKELAAR / MAKER",
    getInTouch: "Neem contact op",
    resume: "CV",
    
    // About
    aboutTitle: "Over Mij",
    aboutP1: "Hey! Ik ben Kilian, een 15-jarige ontwikkelaar uit België. Ik begon met coderen toen ik 7 was, en ik ben er sindsdien verslaafd aan. Ik bouw graag apps, games en websites die echte problemen oplossen.",
    aboutP2: "Als ik niet aan het coderen ben, maak ik muziek. Ik speel piano, gitaar, bas, drums, en ik produceer mijn eigen tracks. Muziek en code delen iets speciaals - beide gaan over het creëren van iets uit niets.",
    aboutP3: "Ik hou ook van koken en experimenteren in de keuken. Of het nu code, muziek of eten is, ik ben altijd aan het leren en iets nieuws aan het maken.",
    aboutMusic: "MUZIEK",
    aboutMusicText: "Piano • Gitaar • Bas • Drums",
    aboutInterests: "INTERESSES",
    aboutInterestsText: "Koken • Game Dev • Muziekproductie",
    aboutAge: "LEEFTIJD",
    aboutAgeText: "15 jaar oud",
    
    // Projects
    projectsTitle: "Mijn Projecten",
    sourceCode: "Broncode",
    liveDemo: "Live Demo",
    comingSoon: "Binnenkort",
    code: "CODE",
    live: "LIVE",
    
    // Skills
    skillsTitle: "Vaardigheden & Technologieën",
    
    // Experience
    experienceTitle: "Mijn Reis",
    
    // Contact
    contactTitle: "Neem Contact Op",
    contactDescription: "Heb je een project in gedachten? Laten we samenwerken om iets geweldigs te creëren.",
    contactEmail: "EMAIL",
    namePlaceholder: "JOUW NAAM",
    emailPlaceholder: "JOUW EMAIL",
    messagePlaceholder: "JOUW BERICHT",
    send: "Verzenden",
    successMessage: "Bedankt voor je bericht! We nemen zo snel mogelijk contact met je op.",
    
    // Footer
    footerRights: "Alle rechten voorbehouden.",
    footerAbout: "Over deze site",
    footerTech: "Gebouwd met React, Next.js, TypeScript, Tailwind CSS, Framer Motion, en gehost op Vercel.",
  },
  de: {
    // Navbar
    home: "Startseite",
    about: "Über",
    projects: "Projekte",
    skills: "Fähigkeiten",
    experience: "Erfahrung",
    contact: "Kontakt",
    
    // Intro
    introTitle: "Kilian Balaguer",
    introSubtitle: "15-jähriger Entwickler & Schöpfer",
    introDescription: "Ich entwickle Apps, Spiele und Websites. Ich bin leidenschaftlich am Programmieren, Musik und am Erschaffen von Dingen, die wichtig sind.",
    introBadge: "ENTWICKLER / SCHÖPFER",
    getInTouch: "Kontakt aufnehmen",
    resume: "Lebenslauf",
    
    // About
    aboutTitle: "Über Mich",
    aboutP1: "Hey! Ich bin Kilian, ein 15-jähriger Entwickler aus Belgien. Ich habe mit 7 Jahren angefangen zu programmieren und bin seitdem süchtig danach. Ich liebe es, Apps, Spiele und Websites zu bauen, die echte Probleme lösen.",
    aboutP2: "Wenn ich nicht am Programmieren bin, mache ich Musik. Ich spiele Klavier, Gitarre, Bass, Schlagzeug und produziere meine eigenen Tracks. Musik und Code teilen etwas Besonderes - beide geht es darum, etwas aus dem Nichts zu erschaffen.",
    aboutP3: "Ich liebe auch das Kochen und Experimentieren in der Küche. Ob Code, Musik oder Essen, ich lerne immer und erschaffe etwas Neues.",
    aboutMusic: "MUSIK",
    aboutMusicText: "Klavier • Gitarre • Bass • Schlagzeug",
    aboutInterests: "INTERESSEN",
    aboutInterestsText: "Kochen • Game Dev • Musikproduktion",
    aboutAge: "ALTER",
    aboutAgeText: "15 Jahre alt",
    
    // Projects
    projectsTitle: "Meine Projekte",
    sourceCode: "Quellcode",
    liveDemo: "Live Demo",
    comingSoon: "Demnächst",
    code: "CODE",
    live: "LIVE",
    
    // Skills
    skillsTitle: "Fähigkeiten & Technologien",
    
    // Experience
    experienceTitle: "Meine Reise",
    
    // Contact
    contactTitle: "Kontakt",
    contactDescription: "Hast du ein Projekt im Kopf? Lass uns zusammenarbeiten, um etwas Erstaunliches zu schaffen.",
    contactEmail: "E-MAIL",
    namePlaceholder: "DEIN NAME",
    emailPlaceholder: "DEINE E-MAIL",
    messagePlaceholder: "DEINE NACHRICHT",
    send: "Senden",
    successMessage: "Vielen Dank für deine Nachricht! Wir melden uns so schnell wie möglich bei dir.",
    
    // Footer
    footerRights: "Alle Rechte vorbehalten.",
    footerAbout: "Über diese Seite",
    footerTech: "Erstellt mit React, Next.js, TypeScript, Tailwind CSS, Framer Motion und gehostet auf Vercel.",
  },
  es: {
    // Navbar
    home: "Inicio",
    about: "Acerca de",
    projects: "Proyectos",
    skills: "Habilidades",
    experience: "Experiencia",
    contact: "Contacto",
    
    // Intro
    introTitle: "Kilian Balaguer",
    introSubtitle: "Desarrollador y Creador de 15 años",
    introDescription: "Construyo aplicaciones, juegos y sitios web. Me apasiona programar, la música y crear cosas que importan.",
    introBadge: "DESARROLLADOR / CREADOR",
    getInTouch: "Ponte en contacto",
    resume: "Currículum",
    
    // About
    aboutTitle: "Acerca de Mí",
    aboutP1: "¡Hola! Soy Kilian, un desarrollador de 15 años de Bélgica. Empecé a programar cuando tenía 7 años y he estado enganchado desde entonces. Me encanta crear aplicaciones, juegos y sitios web que resuelven problemas reales.",
    aboutP2: "Cuando no estoy programando, estoy haciendo música. Toco piano, guitarra, bajo, batería y produzco mis propias pistas. La música y el código comparten algo especial: ambos tratan de crear algo de la nada.",
    aboutP3: "También me encanta cocinar y experimentar en la cocina. Ya sea código, música o comida, siempre estoy aprendiendo y creando algo nuevo.",
    aboutMusic: "MÚSICA",
    aboutMusicText: "Piano • Guitarra • Bajo • Batería",
    aboutInterests: "INTERESES",
    aboutInterestsText: "Cocina • Dev de Juegos • Producción Musical",
    aboutAge: "EDAD",
    aboutAgeText: "15 años",
    
    // Projects
    projectsTitle: "Mis Proyectos",
    sourceCode: "Código Fuente",
    liveDemo: "Demo en Vivo",
    comingSoon: "Próximamente",
    code: "CÓDIGO",
    live: "DEMO",
    
    // Skills
    skillsTitle: "Habilidades y Tecnologías",
    
    // Experience
    experienceTitle: "Mi Viaje",
    
    // Contact
    contactTitle: "Contacto",
    contactDescription: "¿Tienes un proyecto en mente? Trabajemos juntos para crear algo increíble.",
    contactEmail: "CORREO",
    namePlaceholder: "TU NOMBRE",
    emailPlaceholder: "TU CORREO",
    messagePlaceholder: "TU MENSAJE",
    send: "Enviar",
    successMessage: "¡Gracias por contactarnos! Te responderemos lo antes posible.",
    
    // Footer
    footerRights: "Todos los derechos reservados.",
    footerAbout: "Acerca de este sitio",
    footerTech: "Creado con React, Next.js, TypeScript, Tailwind CSS, Framer Motion y alojado en Vercel.",
  },
  ko: {
    // Navbar
    home: "홈",
    about: "소개",
    projects: "프로젝트",
    skills: "기술",
    experience: "경험",
    contact: "연락",
    
    // Intro
    introTitle: "킬리안 발라게르",
    introSubtitle: "15세 개발자 & 크리에이터",
    introDescription: "앱, 게임, 웹사이트를 만듭니다. 코딩, 음악, 의미 있는 것들을 만드는 것에 열정적입니다.",
    introBadge: "개발자 / 크리에이터",
    getInTouch: "연락하기",
    resume: "이력서",
    
    // About
    aboutTitle: "소개",
    aboutP1: "안녕하세요! 저는 벨기에에서 온 15세 개발자 킬리안입니다. 7살 때 코딩을 시작했고 그 이후로 계속 빠져있습니다. 실제 문제를 해결하는 앱, 게임, 웹사이트를 만드는 것을 좋아합니다.",
    aboutP2: "코딩하지 않을 때는 음악을 만듭니다. 피아노, 기타, 베이스, 드럼을 연주하고 제 트랙을 프로듀싱합니다. 음악과 코드는 특별한 공통점이 있습니다 - 둘 다 무에서 유를 창조하는 것입니다.",
    aboutP3: "요리하고 주방에서 실험하는 것도 좋아합니다. 코드든, 음악이든, 음식이든, 저는 항상 배우고 새로운 것을 만들고 있습니다.",
    aboutMusic: "음악",
    aboutMusicText: "피아노 • 기타 • 베이스 • 드럼",
    aboutInterests: "관심사",
    aboutInterestsText: "요리 • 게임 개발 • 음악 프로덕션",
    aboutAge: "나이",
    aboutAgeText: "15세",
    
    // Projects
    projectsTitle: "내 프로젝트",
    sourceCode: "소스 코드",
    liveDemo: "라이브 데모",
    comingSoon: "곧 출시",
    code: "코드",
    live: "라이브",
    
    // Skills
    skillsTitle: "기술 및 도구",
    
    // Experience
    experienceTitle: "나의 여정",
    
    // Contact
    contactTitle: "연락하기",
    contactDescription: "프로젝트가 있으신가요? 함께 멋진 것을 만들어봅시다.",
    contactEmail: "이메일",
    namePlaceholder: "이름",
    emailPlaceholder: "이메일",
    messagePlaceholder: "메시지",
    send: "보내기",
    successMessage: "연락해 주셔서 감사합니다! 최대한 빨리 답변드리겠습니다.",
    
    // Footer
    footerRights: "모든 권리 보유.",
    footerAbout: "이 사이트에 대해",
    footerTech: "React, Next.js, TypeScript, Tailwind CSS, Framer Motion으로 제작되었으며 Vercel에서 호스팅됩니다.",
  },
} as const;
