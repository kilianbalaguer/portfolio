export const projectTranslations = {
  en: {
    aerobrowser: {
      title: "AeroBrowser",
      description: "A modern, fast, and privacy-focused browser for macOS, iOS, and iPadOS. Built with Swift and SwiftUI for optimal performance.",
    },
    voltaire: {
      title: "Voltaire",
      description: "A local AI app built with Swift and MLX for iPhone first, then Mac and iPad, with support for models from Gemma to Mistral, DeepSeek, Qwen, and more.",
    },
  },
  fr: {
    aerobrowser: {
      title: "AeroBrowser",
      description: "Un navigateur moderne, rapide et axé sur la confidentialité pour macOS, iOS et iPadOS. Construit avec Swift et SwiftUI pour des performances optimales.",
    },
    voltaire: {
      title: "Voltaire",
      description: "Une application d'IA locale créée avec Swift et MLX pour iPhone d'abord, puis Mac et iPad, avec des modèles de Gemma à Mistral, DeepSeek, Qwen, et plus encore.",
    },
  },
  ar: {
    aerobrowser: {
      title: "AeroBrowser",
      description: "متصفح حديث وسريع ويركز على الخصوصية لنظام macOS و iOS و iPadOS. تم بناؤه باستخدام Swift و SwiftUI للأداء الأمثل.",
    },
    voltaire: {
      title: "Voltaire",
      description: "تطبيق ذكاء اصطناعي محلي مبني بـ Swift و MLX للـ iPhone أولاً، ثم Mac و iPad، مع دعم لنماذج من Gemma إلى Mistral و DeepSeek و Qwen والمزيد.",
    },
  },
  nl: {
    aerobrowser: {
      title: "AeroBrowser",
      description: "Een moderne, snelle en privacy-gerichte browser voor macOS, iOS en iPadOS. Gebouwd met Swift en SwiftUI voor optimale prestaties.",
    },
    voltaire: {
      title: "Voltaire",
      description: "Een lokale AI-app gebouwd met Swift en MLX voor iPhone eerst, daarna Mac en iPad, met ondersteuning voor modellen van Gemma tot Mistral, DeepSeek, Qwen en meer.",
    },
  },
  de: {
    aerobrowser: {
      title: "AeroBrowser",
      description: "Ein moderner, schneller und datenschutzorientierter Browser für macOS, iOS und iPadOS. Mit Swift und SwiftUI für optimale Leistung gebaut.",
    },
    voltaire: {
      title: "Voltaire",
      description: "Eine lokale KI-App, gebaut mit Swift und MLX für iPhone zuerst, später Mac und iPad, mit Unterstützung für Modelle von Gemma bis Mistral, DeepSeek, Qwen und mehr.",
    },
  },
  es: {
    aerobrowser: {
      title: "AeroBrowser",
      description: "Un navegador moderno, rápido y enfocado en la privacidad para macOS, iOS e iPadOS. Construido con Swift y SwiftUI para un rendimiento óptimo.",
    },
    voltaire: {
      title: "Voltaire",
      description: "Una app local de IA creada con Swift y MLX para iPhone primero, y luego Mac e iPad, con soporte para modelos desde Gemma hasta Mistral, DeepSeek, Qwen y más.",
    },
  },
  ko: {
    aerobrowser: {
      title: "AeroBrowser",
      description: "macOS, iOS 및 iPadOS를 위한 현대적이고 빠르며 개인정보 보호에 중점을 둔 브라우저입니다. 최적의 성능을 위해 Swift와 SwiftUI로 구축되었습니다.",
    },
    voltaire: {
      title: "Voltaire",
      description: "Swift와 MLX로 만든 로컬 AI 앱으로, 먼저 iPhone용으로 시작하고 이후 Mac과 iPad를 지원하며 Gemma부터 Mistral, DeepSeek, Qwen 모델과 더 많은 모델을 지원합니다.",
    },
  },
} as const;

export const experienceTranslations = {
  en: [
    {
      title: "Front-End Development",
      location: "Rijnsburg, Netherlands",
      description: "Age 11 — began learning HTML, JavaScript and CSS using online resources.",
      date: "2021",
    },
    {
      title: "Next.js (Front-End)",
      location: "Rijnsburg, Netherlands",
      description: "Age 12 — learned Next.js to build modern front-end applications and improve web development skills.",
      date: "2022",
    },
    {
      title: "Kotlin & Android",
      location: "Rijnsburg, Netherlands",
      description: "Age 12 — started Kotlin for Android and later used it in collaborative app projects.",
      date: "2022–2023",
    },
    {
      title: "Swift & Apple Development",
      location: "Rijnsburg, Netherlands",
      description: "Age 12 — learned Swift, C and Storyboard for Apple development; later transitioned to SwiftUI.",
      date: "2022–2023",
    },
    {
      title: "C# & WinUI",
      location: "Rijnsburg, Netherlands",
      description: "Age 12 — learned C# and expanded XML knowledge to build Windows apps with WinUI 3.",
      date: "2022–2023",
    },
    {
      title: "SwiftUI Migration",
      location: "Rijnsburg, Netherlands",
      description: "Age 13 — migrated Apple development to SwiftUI to improve app architecture.",
      date: "2023–2024",
    },
    {
      title: "Hackintosh Build",
      location: "Rijnsburg, Netherlands",
      description: "Age 14 — built my first Hackintosh; gained hardware and macOS experience.",
      date: "2024",
    },
    {
      title: "Moved to Marrakech",
      location: "Marrakech, Morocco",
      description: "Age 16 — relocated to Marrakech, Morocco. Continuing to build apps and explore new opportunities.",
      date: "2026",
    },
  ],
  fr: [
    {
      title: "Développement Front-End",
      location: "Rijnsburg, Pays-Bas",
      description: "11 ans — j'ai commencé à apprendre HTML, JavaScript et CSS en autodidacte avec des ressources en ligne.",
      date: "2021",
    },
    {
      title: "Next.js (Front-End)",
      location: "Rijnsburg, Pays-Bas",
      description: "12 ans — j'ai appris Next.js pour créer des interfaces front-end modernes et améliorer mes compétences web.",
      date: "2022",
    },
    {
      title: "Kotlin & Android",
      location: "Rijnsburg, Pays-Bas",
      description: "12 ans — j'ai commencé Kotlin pour Android ; ces connaissances ont ensuite été utiles dans des projets collaboratifs.",
      date: "2022–2023",
    },
    {
      title: "Swift & Développement Apple",
      location: "Rijnsburg, Pays-Bas",
      description: "12 ans — j'ai appris Swift, C et Storyboard pour le développement Apple ; j'ai ensuite migré vers SwiftUI.",
      date: "2022–2023",
    },
    {
      title: "C# & WinUI",
      location: "Rijnsburg, Pays-Bas",
      description: "12 ans — j'ai appris C# et approfondi mes connaissances XML pour créer des applications Windows avec WinUI 3.",
      date: "2022–2023",
    },
    {
      title: "Migration vers SwiftUI",
      location: "Rijnsburg, Pays-Bas",
      description: "13 ans — j'ai migré le développement Apple vers SwiftUI pour améliorer l'architecture de l'application.",
      date: "2023–2024",
    },
    {
      title: "Hackintosh",
      location: "Rijnsburg, Pays-Bas",
      description: "14 ans — j'ai construit mon premier Hackintosh ; j'ai gagné en expérience matériel et macOS.",
      date: "2024",
    },
    {
      title: "Installation à Marrakech",
      location: "Marrakech, Maroc",
      description: "16 ans — installation à Marrakech, Maroc. Je continue à créer des apps et à explorer de nouvelles opportunités.",
      date: "2026",
    },
  ],
  ar: [
    {
      title: "تطوير الواجهة الأمامية",
      location: "رينزبورغ، هولندا",
      description: "11 سنة — بدأت بتعلم HTML و JavaScript و CSS بشكل ذاتي باستخدام الموارد عبر الإنترنت.",
      date: "2021",
    },
    {
      title: "Kotlin و Android",
      location: "رينزبورغ، هولندا",
      description: "12 سنة — بدأت Kotlin لتطوير Android، ثم استخدمت هذه المعرفة في مشاريع تعاونية.",
      date: "2022–2023",
    },
    {
      title: "Swift وتطوير Apple",
      location: "رينزبورغ، هولندا",
      description: "12 سنة — تعلمت Swift و C و Storyboard لتطوير Apple؛ ثم انتقلت إلى SwiftUI.",
      date: "2022–2023",
    },
    {
      title: "C# و WinUI",
      location: "رينزبورغ، هولندا",
      description: "12 سنة — تعلمت C# ووسعت معرفتي بـ XML لبناء تطبيقات Windows باستخدام WinUI 3.",
      date: "2022–2023",
    },
    {
      title: "تعاون جماعي",
      location: "رينزبورغ، هولندا",
      description: "13 سنة — عملت مع مطور آخر على مشاريع تطبيقات وتعلمت أساليب العمل الجماعي.",
      date: "2023",
    },
    {
      title: "الانتقال إلى SwiftUI",
      location: "رينزبورغ، هولندا",
      description: "13 سنة — قمت بترحيل تطوير Apple إلى SwiftUI لتحسين بنية التطبيق.",
      date: "2023–2024",
    },
    {
      title: "بناء Hackintosh",
      location: "رينزبورغ، هولندا",
      description: "14 سنة — بنيت أول Hackintosh لي؛ اكتسبت خبرة في الأجهزة و macOS.",
      date: "2024",
    },
    {
      title: "الانتقال إلى مراكش",
      location: "مراكش، المغرب",
      description: "16 سنة — انتقلت إلى مراكش، المغرب. أستمر في بناء التطبيقات واستكشاف فرص جديدة.",
      date: "2026",
    },
  ],
  nl: [
    {
      title: "Front-End Development",
      location: "Rijnsburg, Nederland",
      description: "11 jaar — ik begon met het zelfstandig leren van HTML, JavaScript en CSS met online bronnen.",
      date: "2021",
    },
    {
      title: "Next.js (Front-End)",
      location: "Rijnsburg, Nederland",
      description: "12 jaar — ik leerde Next.js om moderne front-ends te bouwen en mijn webdevelopment-vaardigheden te verbeteren.",
      date: "2022",
    },
    {
      title: "Kotlin & Android",
      location: "Rijnsburg, Nederland",
      description: "12 jaar — ik begon met Kotlin voor Android; deze kennis bleek later nuttig in samenwerkingsprojecten.",
      date: "2022–2023",
    },
    {
      title: "Swift & Apple Development",
      location: "Rijnsburg, Nederland",
      description: "12 jaar — ik leerde Swift, C en Storyboard voor Apple Development; later ben ik overgestapt naar SwiftUI.",
      date: "2022–2023",
    },
    {
      title: "C# & WinUI",
      location: "Rijnsburg, Nederland",
      description: "12 jaar — ik leerde C# en breidde mijn XML-kennis uit om Windows-apps te bouwen met WinUI 3.",
      date: "2022–2023",
    },
    {
      title: "Migratie naar SwiftUI",
      location: "Rijnsburg, Nederland",
      description: "13 jaar — ik migreerde Apple-development naar SwiftUI om de app-architectuur te verbeteren.",
      date: "2023–2024",
    },
    {
      title: "Hackintosh Build",
      location: "Rijnsburg, Nederland",
      description: "14 jaar — ik bouwde mijn eerste Hackintosh; ik deed ervaring op met hardware en macOS.",
      date: "2024",
    },
    {
      title: "Verhuizing naar Marrakech",
      location: "Marrakech, Marokko",
      description: "16 jaar — verhuisd naar Marrakech, Marokko. Blijf apps bouwen en nieuwe kansen verkennen.",
      date: "2026",
    },
  ],
  de: [
    {
      title: "Front-End-Entwicklung",
      location: "Rijnsburg, Niederlande",
      description: "11 Jahre — ich begann, HTML, JavaScript und CSS selbstständig mit Online-Ressourcen zu lernen.",
      date: "2021",
    },
    {
      title: "Kotlin & Android",
      location: "Rijnsburg, Niederlande",
      description: "12 Jahre — ich begann mit Kotlin für Android; dieses Wissen war später in kollaborativen Projekten nützlich.",
      date: "2022–2023",
    },
    {
      title: "Swift & Apple-Entwicklung",
      location: "Rijnsburg, Niederlande",
      description: "12 Jahre — ich lernte Swift, C und Storyboard für Apple-Entwicklung; später wechselte ich zu SwiftUI.",
      date: "2022–2023",
    },
    {
      title: "C# & WinUI",
      location: "Rijnsburg, Niederlande",
      description: "12 Jahre — ich lernte C# und erweiterte mein XML-Wissen, um Windows-Apps mit WinUI 3 zu bauen.",
      date: "2022–2023",
    },
    {
      title: "Teamzusammenarbeit",
      location: "Rijnsburg, Niederlande",
      description: "13 Jahre — ich arbeitete mit einem anderen Entwickler an App-Projekten und lernte Teamabläufe kennen.",
      date: "2023",
    },
    {
      title: "Migration zu SwiftUI",
      location: "Rijnsburg, Niederlande",
      description: "13 Jahre — ich migrierte Apple-Entwicklung zu SwiftUI, um die App-Architektur zu verbessern.",
      date: "2023–2024",
    },
    {
      title: "Hackintosh-Build",
      location: "Rijnsburg, Niederlande",
      description: "14 Jahre — ich baute meinen ersten Hackintosh; ich sammelte Erfahrung mit Hardware und macOS.",
      date: "2024",
    },
    {
      title: "Umzug nach Marrakesch",
      location: "Marrakesch, Marokko",
      description: "16 Jahre — Umzug nach Marrakesch, Marokko. Entwickle weiterhin Apps und erkunde neue Möglichkeiten.",
      date: "2026",
    },
  ],
  es: [
    {
      title: "Desarrollo Front-End",
      location: "Rijnsburg, Países Bajos",
      description: "11 años — comencé a aprender HTML, JavaScript y CSS por mi cuenta usando recursos en línea.",
      date: "2021",
    },
    {
      title: "Kotlin & Android",
      location: "Rijnsburg, Países Bajos",
      description: "12 años — comencé con Kotlin para Android; ese conocimiento luego fue útil en proyectos colaborativos.",
      date: "2022–2023",
    },
    {
      title: "Swift & Desarrollo Apple",
      location: "Rijnsburg, Países Bajos",
      description: "12 años — aprendí Swift, C y Storyboard para desarrollo Apple; luego migré a SwiftUI.",
      date: "2022–2023",
    },
    {
      title: "C# & WinUI",
      location: "Rijnsburg, Países Bajos",
      description: "12 años — aprendí C# y amplié mi conocimiento de XML para crear apps de Windows con WinUI 3.",
      date: "2022–2023",
    },
    {
      title: "Colaboración en equipo",
      location: "Rijnsburg, Países Bajos",
      description: "13 años — trabajé con otro desarrollador en proyectos de apps y aprendí cómo funciona el trabajo en equipo.",
      date: "2023",
    },
    {
      title: "Migración a SwiftUI",
      location: "Rijnsburg, Países Bajos",
      description: "13 años — migré el desarrollo Apple a SwiftUI para mejorar la arquitectura de la app.",
      date: "2023–2024",
    },
    {
      title: "Construí un Hackintosh",
      location: "Rijnsburg, Países Bajos",
      description: "14 años — construí mi primer Hackintosh; gané experiencia en hardware y macOS.",
      date: "2024",
    },
    {
      title: "Mudanza a Marrakech",
      location: "Marrakech, Marruecos",
      description: "16 años — mudanza a Marrakech, Marruecos. Continúo creando apps y explorando nuevas oportunidades.",
      date: "2026",
    },
  ],
  ko: [
    {
      title: "프론트엔드 개발",
      location: "레인스버그, 네덜란드",
      description: "11살 — 온라인 자료로 HTML, JavaScript, CSS를 독학하기 시작했습니다.",
      date: "2021",
    },
    {
      title: "Kotlin & Android",
      location: "레인스버그, 네덜란드",
      description: "12살 — Android 개발을 위해 Kotlin을 시작했고, 나중에는 협업 프로젝트에서 이 지식을 활용했습니다.",
      date: "2022–2023",
    },
    {
      title: "Swift & Apple 개발",
      location: "레인스버그, 네덜란드",
      description: "12살 — Apple 개발을 위해 Swift, C, Storyboard를 배웠고 이후 SwiftUI로 전환했습니다.",
      date: "2022–2023",
    },
    {
      title: "C# & WinUI",
      location: "레인스버그, 네덜란드",
      description: "12살 — C#을 배우고 WinUI 3를 위한 XML 지식을 확장했습니다.",
      date: "2022–2023",
    },
    {
      title: "팀 협업",
      location: "레인스버그, 네덜란드",
      description: "13살 — 다른 개발자와 앱 프로젝트를 함께 진행하며 팀워크를 배웠습니다.",
      date: "2023",
    },
    {
      title: "SwiftUI로 이전",
      location: "레인스버그, 네덜란드",
      description: "13살 — 앱 아키텍처를 개선하기 위해 Apple 개발을 SwiftUI로 이전했습니다.",
      date: "2023–2024",
    },
    {
      title: "Hackintosh 제작",
      location: "레인스버그, 네덜란드",
      description: "14살 — 첫 Hackintosh를 제작했고 하드웨어 및 macOS 경험을 쌓았습니다.",
      date: "2024",
    },
    {
      title: "마라케시로 이주",
      location: "마라케시, 모로코",
      description: "16살 — 모로코 마라케시로 이주했습니다. 앱을 계속 만들고 새로운 기회를 탐색하고 있습니다.",
      date: "2026",
    },
  ],
} as const;
